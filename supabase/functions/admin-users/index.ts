...
serve(withCors(async (req) => {
  try {
    ...
    if (action === 'delete') {
      ...
      // Historical records (inspections, reports, comments, etc.) are preserved
      // on delete: their FKs are ON DELETE SET NULL, so the user can always be
      // removed and the records keep existing with a null author.
